...
const freeTicketCheckbox = document.querySelector('input[type="checkbox"]');
...
const chartData = {
  daily: {
    revenue: {
      ...
      data: Array.from({ length: 24 }, () => Math.floor(Math.random() * 5000)),
    },
    categories: {
      data: [15, 12, 8, 7, 5, 3],
    },
  ...
  weekly: {
    revenue: {
      ...
      data: Array.from({ length: 7 }, () => Math.floor(Math.random() * 20000)),
    },
    categories: {
...
function initializeCharts() {
  ...
  charts.category = new Chart(document.getElementById('categoryChart'), {
    type: 'doughnut',
    data: {
      labels: ['Music', 'Tech', 'Sports', 'Art', 'Food & Drink', 'Business'],
      datasets: [
        {
...
function updateCharts(period) {
  ...
  charts.category.data.datasets[0].data = chartData[period].categories.data;
